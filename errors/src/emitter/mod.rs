...
pub trait Emitter {
    /// Emit the error `err`.
    fn emit_err(&mut self, err: ConlangError);
...
/// A buffer of `ConlangError`s.
pub type ErrBuffer = Buffer<ConlangError>;
...
impl Handler {
    ...
    pub fn with<T>(logic: impl for<'a> FnOnce(&'a Handler) -> Result<T, ConlangError>) -> Result<T, ErrBuffer> {
        let (handler, buf) = Handler::new_with_buf();
        handler.extend_if_error(logic(&handler)).map_err(|_| buf.extract_errs())
    }

    /// Emit the error `err`.
    pub fn emit_err<E: Into<ConlangError>>(&self, err: E) {
        self.inner.borrow_mut().emit_err(err.into());
    }

    /// Emits the error `err`.
    /// This will immediately abort compilation.
    pub fn fatal_err(&self, err: ConlangError) -> ! {
    ...
    pub fn last_err(&self) -> Result<(), Box<ConlangError>> {
        if let Some(code) = self.inner.borrow().last_emitted_err_code() {
            Err(Box::new(ConlangError::LastErrorCode(code)))
        } else {
            Ok(())
        }
    }
    ...
    pub fn extend_if_error<T>(&self, res: Result<T, ConlangError>) -> Result<T, ()> {
        match res {
...
mod tests {
    ...
    use conlang_span::{session_globals::create_session_if_not_set_then, Span};
    ...
    fn buffer_works() {
        create_session_if_not_set_then(|_| {
            ...
            let res: Result<(), _> = Handler::with(|h| {
                ...
                Err(ParserError::unexpected_eof(s).into())
            });
