...
use conlang_span::{session_globals::with_session_globals, source_map::SpanLocation, Span};
...
pub struct Formatted {
    ...
    pub span: Span,
    /// The backtrace to track where the Conlang error originated.
...
impl fmt::Display for Formatted {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let conlang_backtrace = std::env::var("CONLANG_BACKTRACE").unwrap_or_default().trim().to_owned();
        match conlang_backtrace.as_ref() {
            "1" => {
